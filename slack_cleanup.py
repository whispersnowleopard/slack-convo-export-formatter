...
def parse_slack_export(text: str, unknown_name: str = "Unknown") -> List[Tuple[str, str, str]]:
    ...
    Handles two Slack formats:
    1. DM format:
       Name
         Timestamp (with AM/PM)
       Message content
    2. Channel thread format:
       Name  [Timestamp]
       Message content
    
    Args:
        text: Raw Slack export text
        unknown_name: Name to use when author can't be determined
    """
    messages = []
    lines = text.split('\n')
    i = 0
    ...
    while i < len(lines):
        ...
        if match:
            ...
            while i < len(lines):
                ...
                # These are continuation messages from the same author
                cleaned_line = re.sub(r'^\[(\d{1,2}:\d{2}\s*[AP]M)\]', '', next_line.rstrip())
                
                # Add to message if there's content left after stripping timestamp
        ...
        # Format 2 continued: If line is not indented and not empty, might be a name (for DM format)
        if not line.startswith(' ') and stripped:
            ...
            # Otherwise, assume it's a name if it's not too long and not a system message
...
def generate_frontmatter(messages: List[Tuple[str, str, str]], 
    ...
    for mention in set(mentions):
        # Try to find a participant whose name matches the mention
        mention_lower = mention.lower()
        for participant in participants:
            # Normalize participant name (remove spaces, punctuation, brackets)
            participant_clean = re.sub(r'[^\w]', '', participant.lower())
            
            # Split participant name into words for better matching
            participant_words = participant.lower().split()
            
            # Match if:
            # 1. mention is substring of cleaned participant name
            # 2. cleaned participant name is substring of mention
            # 3. mention starts with first 3 chars of participant name (fuzzy match)
            # 4. mention starts with first 3 chars of any word in participant name
            ...
            if mention_lower in participant_clean or participant_clean in mention_lower:
                ...
                # Check if they share first 3 characters
                if mention_lower[:3] == participant_clean[:3]:
                    matches = True
                # Check if mention matches any word in participant name
    ...
    # Get date from first message if possible
    ...
    for i, p in enumerate(participants):
        ...
        # Add blank line between participants (except after the last one)
...
def generate_filename(messages: List[Tuple[str, str, str]],
    ...
    if conversation_type == "dm":
        # Get unique participants (excluding "Unknown")
        participants = sorted(set(name for name, _, _ in messages if name != "Unknown"))
        if len(participants) > 3:
            # Too many people, use generic name
            name_part = "group-dm"
        else:
        ...
        if channel_name:
            ...
            # No channel name provided, use participants
            participants = sorted(set(name for name, _, _ in messages if name != "Unknown"))
            if len(participants) > 3:
                name_part = "channel"
            else:
...
def format_as_markdown(messages: List[Tuple[str, str, str]], 
    ...
    if include_frontmatter:
        ...
        # Add visible header section for human readability
        participants = sorted(set(name for name, _, _ in messages))
        participant_list = ", ".join(participants)
        
        # Use subject if provided, otherwise placeholder
